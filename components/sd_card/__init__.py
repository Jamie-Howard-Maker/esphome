    ...
    if config[CONF_INTERFACE] == "sdspi":
        for pin in (
            ...
            if pin not in config:
                raise cv.Invalid(
                    f"{pin} is required when interface=sdspi"
                )
        ...
        for pin in (
            ...
            if pin not in config:
                raise cv.Invalid(
                    f"{pin} is required when interface=sdmmc"
                )
    ...
    cv.Schema(
        {
            ...
            cv.Optional(CONF_INTERFACE, default="sdmmc"):
                cv.one_of("sdmmc", "sdspi"),

            cv.Required(CONF_CLK_PIN):
                pins.internal_gpio_output_pin_number,

            # SDMMC
            cv.Optional(CONF_CMD_PIN):
                pins.internal_gpio_output_pin_number,

            cv.Optional(CONF_DATA0_PIN):
                pins.internal_gpio_pin_number,

            cv.Optional(CONF_DATA1_PIN):
                pins.internal_gpio_pin_number,

            cv.Optional(CONF_DATA2_PIN):
                pins.internal_gpio_pin_number,

            cv.Optional(CONF_DATA3_PIN):
                pins.internal_gpio_pin_number,

            # SDSPI
            cv.Optional(CONF_MOSI_PIN):
                pins.internal_gpio_output_pin_number,

            cv.Optional(CONF_MISO_PIN):
                pins.internal_gpio_pin_number,

            cv.Optional(CONF_CS_PIN):
                pins.internal_gpio_output_pin_number,
...
if config[CONF_INTERFACE] == "sdspi":
    ...
    if not config[CONF_MODE_1BIT]:
        cg.add(var.set_data1_pin(config[CONF_DATA1_PIN]))ta2_pin(config[CONF_DATA2_PIN]))
